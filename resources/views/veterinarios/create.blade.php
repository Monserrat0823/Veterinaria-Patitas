  ...
  <form action="{{ route('admin.veterinarios.store') }}" method="POST">
    ...
    <x-wire-card>
      ...
      <div class="flex items-center gap-4 mb-6 pb-4 pt-4 border-b border-gray-200">
        <div class="flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-br from-teal-400 to-emerald-600 shadow-lg">
          <i class="fas fa-user-md text-white text-2xl"></i>
        </div>
        <div>
          <h2 class="text-xl font-bold text-gray-800">Nuevo Veterinario</h2>
          <p class="text-sm text-gray-500">Complete los datos del veterinario</p>
        </div>
      </div>
      ...
      <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pl-9">
          <div>
            ...
            <x-wire-input name="nombre" placeholder="Ej: Dr. Juan Pérez" value="{{ old('nombre') }}" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
              <i class="fas fa-briefcase-medical text-teal-500 mr-1"></i> Especialidad
            </label>
            <x-wire-input name="especialidad" placeholder="Ej: Cirugía General" value="{{ old('especialidad') }}" />
          </div>
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pl-9">
          <div>
            ...
            <x-wire-input name="telefono" placeholder="Ej: 555-1234" value="{{ old('telefono') }}" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
              <i class="fas fa-envelope text-blue-500 mr-1"></i> Correo Electrónico
            </label>
            <x-wire-input name="correo_electronico" type="email" placeholder="correo@ejemplo.com" value="{{ old('correo_electronico') }}" />
          </div>
        </div>
      </div>
      ...
      <x-slot name="footer">
        <div class="flex justify-end gap-x-3">
            ...
            <x-wire-button type="submit" blue>
              <i class="fas fa-save mr-2"></i> Guardar Veterinario
            </x-wire-button>
        </div>
      </x-slot>
    </x-wire-card>
