<x-admin-layout title="Usuarios" :breadcrumbs="[
  ...
  <x-slot name="action">
    <x-wire-button blue href="{{ route('admin.users.create') }}" class="shadow-sm font-bold">
  ...
  <div class="p-6 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden font-sans" x-data="{ search: '' }">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 pb-6 border-b border-gray-100">
          <div class="flex items-center gap-4">
              ...
              <div>
                  <h2 class="text-xl font-bold text-gray-900 tracking-tight">Gestión de Usuarios del Sistema</h2>
                  <p class="text-xs text-gray-500 mt-0.5 font-medium">Administre las cuentas de usuario y sus roles asignados</p>
              </div>
          </div>
          ...
          <div class="w-full sm:w-80">
              <div class="relative">
                  ...
                  <input type="text" x-model="search" placeholder="Buscar usuario por nombre..." 
                      ...
                      <tr class="hover:bg-blue-50/30 transition-colors" x-show="search === '' || '{{ strtolower($row->name ?? '') }}'.includes(search.toLowerCase())">
                          @foreach($columns as $column)
                              <td class="px-6 py-4 text-sm font-medium text-gray-800">
                                  ...
                                  @elseif($column['field'] === 'roles')
                                      <span class="px-2.5 py-1 text-xs font-bold rounded-full bg-blue-100 text-blue-800 border border-blue-200">
                                          {{ $column['format']($row->roles) }}
                                      </span>
                                  @elseif(isset($column['format']))
                                      <span class="text-gray-600">{{ $column['format']($row->{$column['field']}) }}</span>
                                  @else
                                      <span class="text-gray-600">{{ $row->{$column['field']} }}</span>
                                  @endif
                              </td>
                          @endforeach
                          <td class="px-6 py-4 text-sm text-right">
                              <div class="flex items-center justify-end gap-2">
                                  <a href="{{ route('admin.users.edit', $row) }}" class="p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition-colors" title="Editar Usuario">
                                      <i class="fas fa-edit text-base"></i>
                                  </a>
                                  <form action="{{ route('admin.users.destroy', $row) }}" method="POST" class="inline" id="delete-form-{{ $row->id }}">
                                      @csrf
                                      @method('DELETE')
                                      <button type="button" onclick="confirmarEliminar({{ $row->id }})" class="p-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition-colors cursor-pointer" title="Eliminar Usuario">
