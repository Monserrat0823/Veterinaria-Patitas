...
<div class="max-w-3xl mx-auto font-sans">
  ...
  <div class="bg-white p-6 rounded-2xl shadow-xs border border-gray-200 mb-6 flex items-center justify-between gap-4">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h2 class="text-xl font-bold text-gray-900 tracking-tight">Crear Nuevo Rol</h2>
        <p class="text-xs text-gray-500 mt-0.5">Defina un nuevo perfil de acceso para los usuarios de la clínica</p>
      </div>
    </div>
  ...
  <div class="bg-white p-8 rounded-2xl shadow-xs border border-gray-200">
    <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" id="name" name="name" placeholder="Ej. Administrador, Recepcionista, Médico..." value="{{ old('name') }}" 
        ...
        @enderror
        <p class="text-[11px] text-gray-400 mt-1.5 font-medium">El nombre debe ser único para evitar conflictos de asignación en el sistema.</p>
      </div>
